...
# ---- cn-site adaptations ----
def test_cn_selfhosted_image_root_relative():
    md, imgs = w.html_to_markdown(
        '<img src="http://home.wangjianshuo.com/cn/2017/01/photo.jpg"/>')
    assert '![](/cn/2017/01/photo.jpg)' in md, repr(md)
    assert imgs == ['http://home.wangjianshuo.com/cn/2017/01/photo.jpg'], repr(imgs)

def test_www_selfhosted_image_root_relative():
    md, _ = w.html_to_markdown(
        '<img src="http://www.wangjianshuo.com/about/logos/dot.gif"/>')
    assert '![](/about/logos/dot.gif)' in md, repr(md)

def test_htm_permalink_preserved_verbatim():
    assert w._norm_url('https://home.wangjianshuo.com/cn/20120830_eaeaeaeccc.htm',
                       '5', 'post') == '/cn/20120830_eaeaeaeccc.htm'

def test_extensionless_url_still_gets_trailing_slash():
    assert w._norm_url('https://huixianju.cn/archives/1', '1', 'post') == '/archives/1/'


# ---- CJK slug -> /cn/<date>_p<id>.htm ----
def test_cjk_slug_rewritten_to_pid():
    new, orig = w._cjk_slug_to_pid('/cn/20121101_%e6%80%8e%e4%b9%88.htm', '561')
    assert new == '/cn/20121101_p561.htm', new
    assert orig == '/cn/20121101_%e6%80%8e%e4%b9%88.htm'

def test_ascii_letter_slug_untouched():
    new, orig = w._cjk_slug_to_pid('/cn/20080325_aeccaecc.htm', '42')
    assert new == '/cn/20080325_aeccaecc.htm' and orig is None
